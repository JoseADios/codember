...
function validateCheckSum(fileList) {
    ...
    fileList.split('\n').forEach(el => {
        ...
        file.split('').forEach(letter => {
            var count = file.split(letter).length - 1;

            if (count === 1 && !checkReal.includes(letter)) {
                checkReal += letter
            }
        });

        if (checkSum === checkReal) {
            res.push(checkReal)
        }
...
let resultado = validateCheckSum(files)
