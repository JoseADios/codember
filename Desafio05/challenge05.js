...
function getInvalidUsers(users) {
    ...
    usersList.forEach(user => {
        let delUser = user.split(',')

        let id = delUser[0]
        let username = delUser[1]
        let email = delUser[2]
        let age = delUser[3]
        let location = delUser[4]

        // validate id
        if (!isAlphaNumeric(id)) {
            invalidList.push(username)
...
function isAlphaNumeric(word) {
    if (word === '' || !(word.replace(/[A-Z]|[a-z]|[0-9]/g, "") === "")) {
        return false
    } else {
        return true
    }
...
function isNumeric(param) {
    const regex = /^[0-9]*$/;
    const onlyNumbers = regex.test(param);
    return onlyNumbers
}

function isAlpha(word) {
    if (word === '' || (word.replace(/[A-Z]|[a-z]| /g, "") === "")) {
        return true
    } else {
        return false
    }
...
function getUserFirstLetter(usernames) {
    let message = ''
...
let result1 = getInvalidUsers(users)


console.log(getUserFirstLetter(result1));