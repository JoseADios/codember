...
function buscaRepetidos(valores) {
    let miArray = [];
    let palabrasNoRep = [];
    let res = ''

    valores = mensaje.toLocaleLowerCase();
    miArray = valores.split(' ');

    miArray.forEach(item => {
        if (!palabrasNoRep.includes(item)) {
            palabrasNoRep.push(item);
        }
    });

    palabrasNoRep.forEach(el => {
        let cont = 0

        miArray.forEach(repEl => {
            if (el === repEl) {
                cont += 1;
            }
        });
